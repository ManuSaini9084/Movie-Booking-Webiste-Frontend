...
export const AdminShownInModifySection = ({ selectedDate }) => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `http://localhost:7002/adminLatestShowDates`
        );
      ...
      if (selectedShowDate !== "") {
        try {
          const response = await axios.post(
            `http://localhost:7002/adminShowtimes`,
            {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.post(
          `http://localhost:7002/movieReplaceFrom`,
          {
            email: signedPerson.email,
            password: signedPerson.password,
            selectedShowtime,
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.post(
          `http://localhost:7002/movieReplaceTo`,
          {
            email: signedPerson.email,
            password: signedPerson.password,
            selectedShowtime,
